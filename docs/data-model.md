...
- `users/{uid}`: 이메일, 표시 이름, 현재 활성 가구 ID, 사용자별 지출·유통기한 알림 설정
...
- 알림 기본값은 지출과 냉장고 모두 켬이며 3일 전·1일 전·당일 오전 9시에 예약한다.
- 지출 알림은 지정 납부자에게만 노출하고 냉장고 알림은 사용자별 설정을 따른다.
- 항목 또는 사용자 설정 변경 시 Expo Notifications 예약을 자동으로 갱신한다.
