...
- `qa:app-smoke`는 로그인, 마이페이지, 5개 탭, 주요 등록 폼 진입을 실제 Chrome에서 확인한다.
- UI smoke는 저장 버튼을 누르지 않아 Firebase 데이터를 변경하지 않는다.
- 실패 화면은 Jira에 업로드하지 않고 `~/.local/state/jalsarabose/qa-daily-artifacts`에 보관한다.
