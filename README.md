...
- 지출: 월 공동생활비와 2인 부담금, 공과금/생활비 CRUD, 납부 상태, 잔액과 정산 안내
...
현재 상태는 `기능 구현 완료, 외부 릴리즈 준비 중`입니다. 로그인·2인 가구 공유, 홈·캘린더, 지출·월 공동생활비·정산 안내, 냉장고, 로컬 알림 예약 코드는 구현되어 있습니다. 실제 iOS/Android 알림 수신, 릴리즈 Firebase 2계정 스모크, 배포 환경과 스토어 자산은 아직 릴리즈 게이트로 남아 있습니다.

측정 가능한 성공 기준(SC-01~SC-06), 영역별 구현/검증 상태, 필수 릴리즈 게이트(RG-01~RG-05)의 기준 문서는 [docs/product-brief.md](docs/product-brief.md)입니다. 구현 상태를 바꾸는 변경은 이 README와 Notion 전달 문서도 함께 갱신해야 합니다.
...
2. 릴리즈 Firebase에서 2계정 초대·공유·가구 격리 스모크 재실행
3. 릴리즈 후보의 모바일 접근성과 반응형 회귀 확인
4. 배포 환경 구성과 공개 URL 검증
5. 앱 아이콘, splash, 개인정보 처리 안내와 스토어 메타데이터 준비
